...
这正是原文的主要上界之一（原文定理 6.1）。这里的 $\mathrm{O}(f(N))$ 表示忽略常数后不超过 $f(N)$ 这个数量级，$\Omega(f(N))$ 表示至少达到这个数量级，$\Theta(f(N))$ 则表示上下界都是同一个数量级。整套设计虽然公式不少，但主线只有一句话：**把不同大小的内存块看成 $B$ 进制计数器的不同数位，再用冗余数位给 `Grow` 和 `Shrink` 之间留出缓冲区。**
...
Okasaki 在博士论文第 6 章讨论 numerical representations（数值表示）时，总结过一种很有用的看法：数据结构中不同尺寸组件的数量，可以看成一个数的各个数位。插入、删除、合并和拆分组件，分别对应数字的加一、减一、进位和借位[2]。放到本报告中，对应关系如下：
...
本报告第 8 节用自然语言描述 $r=3$ 的同一结构时，已经把更新过程写成“情况三回到情况二、再回到情况一”的级联重查。这个不依赖伪代码的直观描述会自然得到可执行的控制流，也进一步说明原文 Figure 5 的 `else if` 链偏离了该构造本身要求的处理顺序。
...
上面把 $B^{r-1}$ 简化为 $\mathrm{O}(N^{1-1/r})$ 的换算需要 $r$ 是固定常数。如果 $r=r(N)$ 随 $N$ 增长，$4^{r-1}=2^{\Theta(r)}$ 不能再当成固定常数直接省略。这个现象先记在这里；原文对“默认 $r$ 为常数”的正式 caveat 出现在摊还分析末尾，后文再按原文位置说明。
...
本节把一次 item assignment，也就是“把一个元素写到某个位置”，定义为 1 单位实际成本。第一次写入新元素和把旧元素复制到新块，都各算一次。这样定义已经够用，因为分配、释放、改计数器和改指针等基本操作的总数，与“元素写入次数 $+1$”成正比。这里的 $+1$ 不能省：有些普通 Shrink 完全不复制元素，但仍然要执行 $N--$ 和 $n_0--$。
...
在原文引理 6.3 默认的固定 $r$ 情况下，$B^{r-1}=\mathrm{O}(N^{1-1/r})=o(N)$。为了避免动用原文已经“刚好花完”的 $2r$ 预算，可以把每次操作给全局账户的收费改成 $4r$。两次重建之间至少有 $N/2$ 次操作，因此可收到至少 $2rN$；其中不超过 $rN$ 支付重建和全体元素代币，剩下的 $rN$ 足以支付 $\mathrm{O}(B^{r-1})$ 的层账户初始化。收费仍是 $\mathrm{O}(r)$，所以最终的摊还上界不变。

把三本账加起来，每次操作领取的代币仍是 $\mathrm{O}(r)$，因此 Grow 和 Shrink 的摊还成本都是 $\mathrm{O}(r)$。常数 4 没有特殊含义，把收费取大一点只是为跨 Rebuild 的层账户留出余量。
...
上述结论还依赖 `msb` 能在常数时间内算出的假设。论文提到：Fredman-Willard 的方法使用乘法；不使用乘法时，Brodal 的方法需要 $\mathrm{O}(\log\log w)$，其中 $w$ 是一个机器字的位数；Brodnik、Miltersen 和 Munro 还给出了相匹配的下界[1]。作者自己也承认，实际程序中的 $r$ 通常很小，用 $\mathrm{O}(\log r)$ 二分，甚至直接 $\mathrm{O}(r)$ 扫描，可能反而更快。

我认为这是一个很好的批判点：论文证明了“模型允许时可以做到 $\mathrm{O}(1)$”，但它不等于“工程上一定应该照这套位运算实现”。理论上的常数次复杂位运算，实际运行时未必比扫描几个层更快。
...
需要在给出结论前提醒一次：这里的 $r=\Theta(\log N)$ 正是前面摊还分析末尾所说“需要对非常数 $r$ 严格化”的情形。原文直接向定理 6.1 代入 $r=\log N$ 得到这个推论，但没有详写相关常数如何统一控制。
...
这几乎只比保存 $N$ 个元素本身多出对数个机器字，是整个构造最亮眼的结论。不过需要注意，$N+\mathrm{O}(\log N)$ 说的是操作结束后的稳定存储空间；由一般公式可知，此时扩缩容临时冗余 $\mathrm{O}(N^{1-1/r})$ 仍是 $\mathrm{O}(N)$ 量级，不能把两种空间混为一谈。
...
最需要理解的是摊还分析中的三本账：元素账户支付向上合并，层账户支付向下拆分，全局账户支付整体重建。特别是“同一层两次拆分之间至少有 $B^k$ 次 Shrink”，把冗余表示留下缓冲区的直觉和正式的 $\mathrm{O}(r)$ 摊还界连了起来。

第 9 节已经得到稳定冗余 $\mathrm{O}(rN^{1/r})$。当 $r$ 为常数时，这一结果已经与 $\Omega(N^{1/r})$ 的下界匹配；当 $r$ 随 $N$ 增长时，额外的 $r$ 因子不能被大 $O$ 记号隐去。这正是下一节的分块变换需要解决的问题。

## 10 分块变换与稳定冗余优化

前面的多层分块结构已经能够在稳定存储空间、扩缩容临时空间和增删成本之间取得很好的权衡。存放 $N$ 个元素时，它的稳定存储空间是
...
因为大 $O$ 记号会忽略固定的常数因子。此时稳定冗余已经和下界 $\Omega(N^{1/r})$ 相同；扩缩容临时冗余 $\mathrm{O}(N^{1-1/r})$ 也与“稳定冗余和扩缩容临时冗余的乘积至少为 $N$”这一下界相匹配；`Grow` 和 `Shrink` 的 $\mathrm{O}(r)$ 摊还时间则等于 $\mathrm{O}(1)$。因此在 $r$ 为常数时，三项指标都已经达到渐近最优，没有必要继续消除写在公式外面的常数 $r$[1]。

真正有问题的是 $r=r(N)$ 会随着 $N$ 增长的情况。这时 $r$ 不再是可以藏进大 $O$ 的常数。例如取 $r=\Theta(\log N)$ 时，$N^{1/r}$ 是常数，但 $rN^{1/r}$ 仍有 $\Theta(\log N)$ 这么大。上界 $\mathrm{O}(rN^{1/r})$ 和目标 $\mathrm{O}(N^{1/r})$ 之间确实差了一个 $r$ 因子。

原文在开始讨论这个变换时还说明，只需考虑 $r(N)=\mathrm{O}(\log N)$。原因是当 $r$ 已经达到 $\Theta(\log N)$ 时，$N^{1/r}=\mathrm{O}(1)$；再继续增大 $r$，目标额外空间的数量级已经不能比常数更小，所以不会产生新的空间尺度。换句话说，$\log N$ 是值得讨论的最远端，而不是下面定理都能覆盖到的范围。
...
$r=\Theta(\log N)$ 则揭示了原文没有解决的另一段空白：前文的推论给出 $N+\mathrm{O}(\log N)$ 稳定存储空间，而一般下界在这个参数下只有 $N+\Omega(1)$；下面的变换定理并不覆盖这一区间，原文最后的讨论也没有把这个差距补上。这个例子仍然值得保留，但应该把它写成**遗留问题**，而不是定理的应用例子。
...
这样也就能理解原文随后所说的“原文给出的实现都是 standard”：主要的摊还实现确实可以按定义执行，与前面的 Brodnik 摊还版不冲突；不在 standard 类别内的是额外的后台复制技巧。

乍看容易以为，“无法改成最坏 $\mathrm{O}(r)$”是后文 $\Omega(r)$ 摊还下界的推论。其实最坏 $\mathrm{O}(r)$ 本来就会推出摊还 $\mathrm{O}(r)$，与摊还下界 $\Omega(r)$ 完全相容。真正的障碍是原文引言所说的 “background rebuilding cannot be used”：后台重建受到了空间预算限制[1, p. 1356]。
...
$r=2$ 是一个真正的边界情况：此时 $N^{1-1/r}=N^{1/r}=\sqrt N$。Brodnik 结构中的数据元素从不搬运，需要后台复制的只是大小为 $\Theta(\sqrt N)$ 的索引块，新旧索引块暂时并存仍在 $N+\mathrm{O}(\sqrt N)$ 的空间预算内[6]。所以，$r=2$ 的非 standard 技巧和后文 standard 类别的摊还下界是两个需要分开说明的问题。
...
这也意味着下界不能直接解释成“所有能够想象的可伸缩数组都必须如此”。例如长期进行后台增量复制、使用虚拟内存重映射或让多个块共享同一份元素的结构，可能不满足标准实现的定义。它们不一定更好，只是暂时不属于这套下界证明覆盖的范围。原文在最后的开放问题中也明确回到这一限制：作者相信，如果对元素和指针加入不可压缩性假设，下界或许能推广到所有算法；但论文并没有给出这项推广的证明[1, p. 1379]。

### 10.2 分块变换（原文引理 7.2）

现在从任意一个标准的 $(s(N),\mathrm{O}(N))$ 实现出发。第二个分量 $\mathrm{O}(N)$ 表示，一次 Grow 或 Shrink 期间的**总额外空间**至多为线性量级，因而对输入结构的约束很弱。我们另外选择一个非递减函数 $t(N)$，把它当作变换后单个新块允许的最大大小。“非递减”表示随着 $N$ 增大，$t(N)$ 不会反而变小。
...
指针项可以被 $\mathrm{O}(t(N))$ 吸收。最终得到的变换应该分成“一般形式”和“满足额外条件后的简化形式”：
...
如果另外有 $t(N)\ge\sqrt N$，第二个分量才可以简化为 $s(N)+\mathrm{O}(t(N))$。这个式子中，第一个分量是稳定冗余，第二个分量是操作过程中的峰值额外空间。

这里原文的引理 7.2 有一点表述含糊：引理陈述直接给出了简化后的 $s(N)+\mathrm{O}(t(N))$，看起来像对任意 $t(N)$ 都成立；但证明末尾又说可以假设 $t(N)\ge\sqrt N$，正是靠这个条件吸收 $N/t(N)$。因此，阅读时最好把条件明确写进结论，或者像上面一样先保留完整的 $t(N)+N/t(N)$。
...
我们的目标是把稳定冗余降到 $\mathrm{O}(N^{1/r})$。稳定存储空间中新增的指针项是 $\mathrm{O}(N/t(N))$，所以必须有
...
关键技巧是先使用参数为 $2r$ 的多层结构，再对它做切块变换。此时原结构的稳定冗余变成
...
并且 `Get` 和 `Set` 仍为最坏 $\mathrm{O}(1)$。要把它送进变换，还需要检查它是否属于标准实现。

这一步不能只靠一句断言。前面对多层结构的检查列出了 Rebuild 的三个隐含条件：重建后最低层必须可供后续 Shrink 正确删除，层账户必须能重新启动，复制过程必须符合 standard 方式。前两条分别保证伪代码正确性和摊还证明；对当前变换而言，直接决定能否套用原文定义 7.1 的是第三条。
...
最值得注意的是，稳定冗余中的 $r$ 因子已经被完全去掉，但增删时间没有变差。后续下界会进一步说明：如果只考虑标准实现，并把稳定存储空间限制为 $N+\mathrm{O}(N^{1/r})$，那么 $\Omega(r)$ 的 Grow 摊还成本无法避免。这样，上面的 $\mathrm{O}(r)$ 上界就在标准实现这一类别内达到渐近最优。
...
其中最容易漏掉的是第 4 步。直接对参数 $r$ 的结构做切块，只能控制新增指针和峰值空间，原来的 $\mathrm{O}(rN^{1/r})$ 不会凭空消失。参数 $2r$ 的作用，就是先把指数从 $1/r$ 降到 $1/(2r)$，为去掉外面的 $r$ 因子留出足够余量。

至此，上界构造已经完成。剩下的问题是：在相同的空间预算下，$\mathrm{O}(r)$ 的 `Grow` 摊还时间能否继续降低？下面两节把标准实现放宽为增长博弈，并证明即使在这个更宽松的模型中仍需 $\Omega(r)$ 的摊还成本。
