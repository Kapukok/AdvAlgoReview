...
原文 Figure 5 的这一分支存在控制流错误。arXiv v2（2023 年 5 月 29 日版）第 10 页把三个判断写成了同一条 `else if` 链，SIAM 期刊版第 1364 页仍保留了相同写法[1, p. 1364][5, p. 10]。例如取 $r=3,B=4,N=32$，此时 $n_1=8,n_0=4$，Grow 会调用 Combine-Blocks。合并结束后状态变为 $n_1=4,n_0=4$，也就是剩下的 4 个 $B$ 块仍然全满；如果直接执行
...
该 Grow 分支问题同时存在于预印本和 2024 年 SIAM 期刊版。上面的伪代码采用修正后的写法。
...
原文两个版本的 Figure 5 还省略了两处计数器更新。每轮分配出 $B$ 个 $B^i$ 块后，伪代码没有显式写出 `ni <- B`[1, p. 1364][5, p. 10]。正文明确说明新产生了 $B$ 个块，而且下一轮要先把这个数量减一；如果不更新 $n_i$，伪代码无法继续执行。因此上面的整理版补上了这一行。

拆到底层后，还应显式写出 `n0 <- B`，表示最后一个新产生的 $B$ 块是满的；这一行在两个版本的 Figure 5 中同样缺失。两个版本的伪代码随后都让 Shrink 先执行 `n0 <- n0 - 1`。在部分执行轨迹中，上一次释放空 $B$ 块时留下的旧值 $n_0=B$ 会使结果暂时符合预期，但该值已经没有对应的数据块。为使计数器状态与实际块状态一致，整理版补上了这项更新。
...
例如，如果 Rebuild 采用贪心的高层表示，可能得到 $n_2>0$ 但 $n_1=n_0=0$ 的状态。紧接着的第一次 Shrink 就会在第 2 层 Split，需要最多 $3B^2$ 个代币支付复制和元素账户，但该层只从这次 Shrink 收到了 3 个。因此，“第一次 Split 之前也有 $B^k$ 次 Shrink”不能无条件跨过 Rebuild；原文现有分析没有覆盖 Rebuild 后立即发生首次 Split 的情况。
...
综合三类账户，每次操作收取的代币仍为 $\mathrm{O}(r)$，因此 Grow 和 Shrink 的摊还成本都是 $\mathrm{O}(r)$。常数 4 仅用于为跨 Rebuild 的层账户初始化预留足够代币，不影响渐近界。

原文在摊还分析末尾说明，上述分析隐含假设 $r$ 为常数，并称非常数 $r$ 的情形“容易改造”[5, p. 12]，但没有给出具体改造，也没有展开空间常数和代币账户对 $r$ 的依赖关系。因此，原文没有提供 $r=r(N)$ 情形下的完整摊还证明；后文涉及非常数 $r$ 时将保留这一适用范围限制。
...
访问时不能用当前的 $t(N)$ 直接计算 $\lfloor x/t(N)\rfloor$。一个逻辑块可能跨越两次 Rebuild 之间的较长区间，当前 $N$ 与创建时的 $N$ 不同，$t(N)$ 也可能已经变化；使用新阈值解释按旧阈值切出的块会产生错误定位，而且除以任意的 $t(N)$ 还需要一般整数除法。原文只说明变换不改变访问时间，没有给出这项实现细节；为实现该访问界，可以为每个逻辑块保存固定的 2 的幂 chunk size。
...
原文引理 7.2 的陈述直接给出简化后的 $s(N)+\mathrm{O}(t(N))$，而证明末尾使用 $t(N)\ge\sqrt N$ 吸收 $N/t(N)$。因此，该简化形式依赖这一附加条件；在一般情形下应保留完整的 $t(N)+N/t(N)$。
...
Combine 和 Split 的伪代码按数据块顺序整块复制，每个源块复制完就立即释放，因此符合 standard 定义。Rebuild 在原文中只作为过程名称出现，没有给出伪代码；作者关于所有实现均为 standard 的陈述隐含假设 Rebuild 同样采用“顺序复制、每个源块复制完就立即释放”的实现。在这一假设下，参数为 $2r$ 的结构满足变换的输入条件。
