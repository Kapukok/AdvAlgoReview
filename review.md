...
前两节给出了稳定冗余和更新时间的上界。为了能够证明标准实现 `Grow` 操作摊还成本的下界，原文 §8 提出了一个增长博弈，具体定义如下：
...
这一节的证明路线可以先概括如下：原文引理 8.4 先把到达某个最终状态 $\mathbf{a}$ 的成本拆成若干规模更小的 $C_{a_i-1,i}$ 子问题；原文命题 8.8 处理 $n=1$ 的基例；原文命题 8.10、8.11、8.12 再完成归纳步骤，其中 8.10 说明所有落在二项式上下界之间的状态都达到同一个最优成本，8.11 说明如果某个坐标越界，就必然存在另一个方向的越界坐标，8.12 说明可以把这种越界状态改得更便宜，从而排除其最优性。这样就能证明原文定理 8.6，并由 8.6 推出原文推论 8.13。
...
2. 允许合并任意一组块。取一个下标集合 $I=\{i_1,i_2,\dots,i_s\}\subseteq[k]$，其中 $i_1<i_2<\dots<i_r$，然后将 $A_{i_1},A_{i_2},\dots,A_{i_s}$ 合并到 $A_{i_s}$ 并加入新元素，也就是 $a_{i_s}\leftarrow1+\sum_{l=1}^sa_{i_l}$，对所有 $l\in[s-1],a_{i_l}\leftarrow0$。将这种操作称为一个 I-move。证明过程和原文引理 8.17 一致，I-move 对降低最小成本也没有帮助。


上述结果给出了增长博弈的最小成本。下一节把真实标准实现映射到参数适当的增长博弈，并由此导出 `Grow` 的摊还时间下界。

## 12 标准实现的摊还时间下界

现在，给定 $N,k,l$ 的情况下，我们可以得到增长博弈的最优解法。

假设 $r=\mathrm{O}(\log N)$，且一个标准实现的可变长数组可用的额外空间为 $\mathrm{O}(rN^{1/r})$。（原文定理 7.3 中给出的额外空间是 $\mathrm{O}(N^{1/r})$；当前讨论的实现允许多出一个 $r$ 因子的冗余，是更宽松的假设。如果即便在更宽松的空间内，标准实现仍然需要 $\Omega(r)$ 摊还成本，那么在更严格的空间内也不可能更优。）令额外空间为 $rN^{1/r}$，忽略常数，因为对于任意 $\alpha>1$，在 $N$ 足够大时都有 $\alpha rN^{1/r}<(r-1)N^{1/(r-1)}$。因为增长博弈使用的额外空间为 $k+l$，所以 $k\leq rN^{1/r},l\leq rN^{1/r}$，放宽下界，取 $k=l=rN^{1/r}$。对这个可变长数组的任何操作都被扩展的 $(N,rN^{1/r},rN^{1/r})$-增长博弈中的操作覆盖了，这意味着真实数据结构的最小成本不会低于对应的、条件更宽松的增长博弈的最小成本。
...
结合原文 §6 的结论。原文 §6 所构造的实现，有增长摊还成本 $\mathrm{O}(r)$。而当前的实现相比原文 §6 实现更宽松，有一个不会更高的下界。合并上下界，有增长摊还成本 $\Theta(r)$。显然这个实现达到了渐近最优。

至此，本文第 5 节给出了稳定冗余与扩缩容临时冗余之间的空间乘积下界；第 8--10 节从具体分块结构出发，逐步给出了达到相应空间目标的上界构造；第 11--12 节则通过增长博弈说明，在原文定义的标准实现中，增长的摊还时间还受到 $\Omega(r)$ 下界约束，最终得到增长摊还界 $\Theta(r)$。
