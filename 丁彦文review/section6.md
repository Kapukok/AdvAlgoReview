...
这是原文伪代码中一个值得指出的错误。本文所依据的 arXiv v2（2023 年 5 月 29 日版）Figure 5 把三个判断写成了同一条 `else if` 链[5]。例如取 $r=3,B=4,N=32$，此时 $n_1=8,n_0=4$，Grow 会调用 Combine-Blocks。合并结束后状态变为 $n_1=4,n_0=4$，也就是剩下的 4 个 $B$ 块仍然全满；如果直接执行
...
版本说明：目前可以直接核对的 arXiv v2 Figure 5 存在该问题[5]。本文未能核对 SIAM 期刊版的 Figure 5，因此不进一步断言期刊版是否已经修正[1]。上面的伪代码采用经过修正、可以实际执行的写法。
...
还有两个值得核对的计数器更新。原论文的伪代码在每轮分配出 $B$ 个 $B^i$ 块后，没有显式写出 `ni <- B`。但是正文明确说新产生了 $B$ 个块，而且下一轮要先把这个数量减一；如果不更新 $n_i$，伪代码无法继续执行。因此上面的整理版补上了这一行。

拆到底层后，还应显式写出 `n0 <- B`，表示最后一个新产生的 $B$ 块是满的。原伪代码后面的 Shrink 先执行 `n0 <- n0 - 1`，之所以在很多执行轨迹上碰巧得到正确答案，是因为上一次释放空 $B$ 块时留下了旧值 $n_0=B$。正确性不应依赖一个已经没有对应块的陈旧计数，所以整理版也把这项更新补全。
...
原文先给出一种更直接的做法：把各层的 $N_k$ 显式保存在辅助信息中，并在 Grow、Shrink、Combine 和 Split 时更新。随后它用 “Alternatively” 引出上面的掩码计算。显式保存只占 $O(r)$ 个机器字，本来就在元数据预算内；两种做法都不改变访问和增删的渐近界。因此，这里只应说论文给出了两个等价选项，不必替作者补一个“为了压进常数个机器字”的动机。
...
这个推论在原文中是直接向 Theorem 6.1 代入 $r=\log N$ 得到的，不存在一个论文已经写出的“调整后版本”。这也使前面的 caveat 更值得注意：结论本身是论文明确声明的，但从固定 $r$ 的展开证明过渡到 $r=\Theta(\log N)$ 时，相关常数如何统一控制，原文没有详写。
