...
进位后第 $i$ 层还留下 $B$ 个小块。下一次即使马上 Shrink，也可以先从这些小块中删除，不必立即把刚生成的大块拆开。这个从 $B$ 到 $2B$ 的区间就是缓冲区，也可以理解为一种“滞后”设计：触发合并和触发拆分的状态不是同一个，中间故意留出一段安全距离。（具体证明其有效的分析见后文）

Okasaki 的数值表示只是帮助我们建立“数位对应数据块”的直觉，本文并没有直接套用他的函数式数据结构。本文真正使用的是针对分块数组设计的规则：允许每层达到 $2B$，进位时只合并其中 $B$ 个块。论文还指出，Kaplan 等人在另一个问题中也使用过相似的冗余 $B$ 进制计数器[4]。
...
第 1 条关系伪代码能否正确执行，第 2 条关系摊还证明是否闭合，第 3 条关系后续变换的适用范围。它们不是同一类要求，但都因 Rebuild 未展开而被原文隐去了。
...
CLRS 将聚集分析（aggregate analysis）、记账法（accounting method）和势能法（potential method）列为三种并列的摊还分析方法，记账法不是聚集分析的变形[3]。记账法也常被叫作 banker’s method。英文中的记账单位叫作 credit，下文统一称为“代币”。它只是分析时虚构出来的钱，不需要真的存进程序，也不占用数据结构的内存。
...
把三本账加起来，每次操作领取的代币仍是 $O(r)$，因此 Grow 和 Shrink 的摊还成本都是 $O(r)$。这个修补不是在声称常数 4 有特殊意义；它只是为跨 Rebuild 的层账户留出明确余量。
