...
% Preamble 增强排版宏包，不改变原文内容

\documentclass[11pt,a4paper]{article}
\usepackage[margin=1in]{geometry}    % 标准页边距
\usepackage{amsmath,amssymb}         % 完整数学符号支持
\usepackage{tabularx}                % 自适应表格
\usepackage{enumitem}                % 列表统一缩进
\usepackage{hyperref}                % 超链接（不影响正文）
...
趣事：第6.1部分的推论6.2中我还在想$r=\log N$时常驻空间为什么不是$O(\log N\cdot N^{1/\log N})$，突然意识到$N^{1/\log N}$是个常数
\end{quote}
以下为AI翻译的文档：
\vspace{1em}

% 论文标题、作者、摘要（替换Markdown标题，内容完全不变）
\title{最优可变长数组}
...
倍增方案仅保证$O(N)$空间复杂度，虽能满足多数场景，但纯扩容场景下最多会浪费50\%已分配内存。更一般地，若扩容系数为$1+\alpha$，内存闲置比例可达$\frac{1}{1+\alpha}$；若缩小$\alpha$降低闲置占比，单次扩容均摊代价会上升至$O(\frac{1+\alpha}{\alpha})$。
...
缩容对称：仅末尾两块全空才释放尾部块；$B=4\sqrt{N}$即$N=B^2/16$时折半重构。两次重构间至少$\Omega(N)$次操作，缩容均摊$O(1)$。

重构仅需$O(\sqrt{N})$临时空间，新旧块逐块分配释放，无需完整保存两份数组。

可通过双尺寸块+后台重构转为最坏时间版本，但复杂度大幅提升；下一节无需重构、元素不移动。

\subsection{Brodnik等人的分层块结构}
Brodnik独立提出分层方案，常驻$N+O(\sqrt{N})$，扩缩容最坏$O(1)$（分配常数时间）；优势：无全局重构、元素不移动。两种变体：
\begin{enumerate}
    \item 基础变体：数据块尺寸$1,2,3,\dots$；前$k$块总容量$\approx k^2/2$，$k=\lceil\frac{\sqrt{8N+1}-1}{2}\rceil$；末块半满，预留一块空块。索引块尺寸$\Theta(\sqrt{N})$，自身朴素动态数组实现。有空位直接写入；无空位分配新块，索引块满则扩容拷贝指针，后台拷贝实现最坏$O(1)$。下标块编号$\ell=\lceil\frac{\sqrt{8i+1}-1}{2}\rceil$，块内偏移$i-\frac{\ell(\ell-1)}{2}$，读写$O(1)$但需要开平方。
    \item 无平方根变体：所有块尺寸为2的幂，引入超块，移位定位规避平方根运算。
\end{enumerate}
...
\end{enumerate}
地址空间$2^{cw}(0<c<1)$时指针开销为$ck$，仅常数变化，不改变阶。

\begin{theorem}[$(s(N),t(N))$分离下界]
任意仅支持扩容、访问的$(s(N),t(N))$实现，满足$s(N)\cdot t(N)\ge N$。
\end{theorem}
\begin{proof}
$N$个元素分散至多$s(N)$块，鸽巢原理存在块尺寸$\ge N/s(N)$。该块分配时刻瞬时额外空间$\ge N/s(N)$；$t(N)$单调，$t(N)\ge N/s(N)$，变形得$s(N)t(N)\ge N$。
证毕。
\end{proof}
\begin{corollary}
...
\section{$r=3$的极简实现：$(O(N^{1/3}),O(N^{2/3}))$方案}
构造常驻$O(N^{1/3})$、临时$O(N^{2/3})$，访问最坏$O(1)$，增删均摊$O(1)$。

核心改造HAT：外层块$N^{2/3}$、索引$N^{1/3}$，半满块嵌套一层HAT，嵌套额外$O((N^{2/3})^{1/2})=O(N^{1/3})$。合并拷贝瞬时$\Omega(N^{2/3})$临时空间，匹配下界。也可基于Brodnik分层构造。

无递归描述：参数$N^{1/3}\le B\le4N^{1/3}$，两类块：
\begin{enumerate}
    \item 大块：尺寸$B^2$，总数约$N/B^2$，永久填满；
    \item 小块：尺寸$B$，至多$2B$个，至多一块半满、至多一块空。
\end{enumerate}
两级索引分别管理大小块；$2B$小块满则合并为一块大块；无小块则拆分大块。所有访问最坏$O(1)$，增删均摊$O(1)$。

\section{对任意$r\ge2$的通用$(O(rN^{1/r}),O(N^{1-1/r}))$实现}
泛化$r=3$构造，无递归，常驻$O(rN^{1/r})$，临时$O(N^{1-1/r})$。

参数$N^{1/r}\le B<4N^{1/r}$，块尺寸$B,B^2,\dots,B^{r-1}$。仅扩容每层最多$B$块；支持扩缩容放宽至每层$2B$块（冗余基数$B$计数器）。
\begin{itemize}
    \item $B^2,\dots,B^{r-1}$块永久填满；
    \item 仅末尾$B$块可半满。
\end{itemize}

扩容逻辑：末尾$B$块有空直接写入；满且不足$2B$分配新$B$块；满$2B$则$B$个同尺寸小块合并高一阶大块，等价基数$B$进位计数。

$N=B^r$时$B$翻倍全局重构；配套$r-1$层索引。常驻上界$N+(2r-1)+2(r-1)B+B=N+O(rN^{1/r})$；合并$B^{r-1}$块临时峰值$O(N^{1-1/r})$。

原生下标访问最坏$O(r)$；6.3节优化至$O(1)$。增删均摊$O(r)$，元素逐层向上合并，重构分摊常数开销。

缩容：每层上限$2B$块；满$2B$合并；无小块时拆分最小大块，均摊仍$O(r)$。

\subsection{6.1 实现伪代码说明}
$n_i$：尺寸$B^i$块数量；$A[i]$对应索引块；$n_0$末尾$B$块已存元素。
\subsubsection{Grow(a) 扩容流程}
\begin{enumerate}
    \item 若$N=B^r$，执行$\text{Rebuild}(2B)$，$B$翻倍；
    \item 若$n_1=2B,n_0=B$（$B$块全满），调用$\text{Combine-Blocks}$合并；
    \item 无空位则分配新$B$块，$n_0$置0；
    \item 写入新元素，$n_0,N$自增1。
\end{enumerate}
$\text{Combine-Blocks}$：最小未满层$k$，自底向上每层$B$小块合并一块大块，调整指针释放旧块。

\subsubsection{Shrink() 缩容流程}
\begin{enumerate}
    \item 若$N=(B/4)^r$，执行$\text{Rebuild}(B/2)$，$B$折半；
    \item 无$B$块调用$\text{Split-Blocks}$拆分大块；
    \item 删除末尾元素，$n_0,N$自减1；$n_0=0$则释放空$B$块。
\end{enumerate}
$\text{Split-Blocks}$：最小非空大块拆多层小块，分配拷贝释放原块。

辅助函数：
\begin{itemize}
    \item $\text{Allocate}(\ell)$：分配长度$\ell$块，返回指针；
    \item $\text{Deallocate}(X)$：释放块$X$；
    \item $\text{Copy}(X,x,Y,y,\ell)$：$X$偏移$x$起$\ell$元素拷贝至$Y$偏移$y$；
    \item $\text{Rebuild}(B')$：以$B'$全局重构整套分层结构。
\end{itemize}

\begin{theorem}
该实现为$(O(rN^{1/r}),O(N^{1-1/r}))$可变长数组；扩容缩容均摊$O(r)$，下标读写最坏$O(1)$。
\end{theorem}

\begin{corollary}
取$r=\log N$，存在实现常驻$N+O(\log N)$，增删均摊$O(\log N)$，访问最坏$O(1)$。
\end{corollary}

\subsection{6.2 增删均摊代价证明（引理6.3）}
操作代价定义为元素拷贝赋值次数。
\begin{enumerate}
    \item 扩容：基础写入代价1；每个底层元素分配$r-1$信用，每合并一层消耗1信用，合并均摊0，单次扩容均摊$O(r)$；
    \item 缩容：每次缩容为每层增加固定信用；拆分拷贝由累积信用覆盖，拆分均摊0，单次缩摊$O(r)$；
    \item 全局重构：两次重构间至少$\Omega(N)$次操作，总拷贝$N$分摊，不增加渐近阶。
\end{enumerate}
综上扩缩容均摊$O(r)$。

\subsection{6.3 $O(1)$下标访问优化（引理6.4）}
假设CPU内置常数$\text{msb}$（最高置位查询），改造结构：
\begin{enumerate}
    \item $B=2^b$（2的幂）；
    \item 总规模冗余$B$进制$N=\sum_{j=0}^{r-1}n_j B^j,\;0\le n_j\le2B$；
    \item 块计数压缩存入两机器字$N^0,N^1$，移位按位提取；
    \item 预计算前缀容量$N_k$，表示尺寸$\le B^{k-1}$总元素。
\end{enumerate}

访问下标$i$步骤：
\begin{enumerate}
    \item 末尾偏移$x=N-1-i$；
    \item $\text{msb}$定位进制最高位，确定块阶$k$；
    \item 移位按位运算计算块内偏移，全程常数操作。
\end{enumerate}
无原生$\text{msb}$时复杂度$O(\log\log w)$，不改变渐近；工程$r$极小，遍历$r$层也实用。

\section{数据结构变换优化常驻空间}
$r(N)$随$N$缓慢增长时，将常驻$O(rN^{1/r})$降至$O(N^{1/r})$，维持$O(r)$均摊、$O(1)$访问。
\begin{definition}[标准$(s(N),t(N))$实现]
满足两条：
\begin{enumerate}
    \item 分配新块时顺序拷贝旧块，拷贝完成释放旧块；
    \item 拷贝结束常驻回到$N+s(N)$。
\end{enumerate}
本文全部实现均为标准实现。
\end{definition}
...
\begin{lemma}
任意标准$(s(N),O(N))$实现，可变换为$(s(N)+O(N/t(N)),s(N)+O(t(N)))$，不改变访问、均摊阶。
\end{lemma}
思路：超大块拆分多段拷贝，新增指针开销$O(N/t(N))$，临时峰值$s(N)+O(t(N))$。

\begin{theorem}
对$r(N)\le\frac{1}{2}\frac{\log N}{\log\log N}$，存在$(O(N^{1/r}),O(N^{1-1/r}))$实现：访问最坏$O(1)$，增删均摊$O(r)$。
\end{theorem}
证明：对$r'=2r$构造取$t(N)=N^{1-1/r}$；约束下$rN^{1/(2r)}\le N^{1/r}$，常驻简化为$O(N^{1/r})$。

\section{扩容博弈：下界抽象模型}
抽象单人博弈刻画标准实现扩容行为，用于证明$\Omega(r)$下界。
\subsection{8.1 $(N,k,\ell)$扩容博弈定义}
参数$N$总插入、$k$最大子数组数、$\ell$单块最大空位。初始$k$空数组$A_1\dots A_k$，存储倒置；$a_i$为$A_i$元素数。约束：
\begin{enumerate}
    \item 空数组全部前置；
    \item 仅首个非空块允许$\ell$空位，其余填满；
    \item 总闲置开销$k+\ell$，对应常驻额外空间。
\end{enumerate}

单次插入三类操作：
\begin{enumerate}
    \item 首块有空直接写入，代价1；
    \item 无空位但有空数组，分配$\ell+1$新块存元素，代价1；
    \item 全部填满：选$i$合并前$i$块为新$A_i$，代价$1+\sum_{j=1}^i a_j$。
\end{enumerate}
$C_{N,k,\ell}$：$N$次插入最小总代价；$A_{N,k,\ell}=C_{N,k,\ell}/N$单次均摊代价。

\subsection{8.2 归约至$\ell=0$无空位}
\begin{lemma}
$N$整除$\ell+1$时，$C_{N,k,\ell}=(\ell+1)C_{N/(\ell+1),k,0}$，均摊代价相等。
\end{lemma}
直观：每$\ell+1$次填满一块，等价以块为单位运行$\ell=0$博弈。

\subsection{8.3 $\ell=0$博弈精确求解}
记$C_{N,k}=C_{N,k,0}$，状态$P_{N,k}$满足$\sum a_i=N$、空前置。
\begin{theorem}
\begin{enumerate}
    \item 若$\binom{n+k-1}{k}-1\le N\le\binom{n+k}{k}-1$，总代价$C_{N,k}=(N+1)n-\binom{n+k}{k+1}$；
    \item 区间内每次新增代价恒为$n$：$C_{N,k}-C_{N-1,k}=n$；
    \item 最优状态充要：$\binom{n+i-2}{i}\le a_i\le\binom{n+i-1}{i},\forall i\in[k]$。
\end{enumerate}
\end{theorem}
\begin{corollary}
\begin{enumerate}
    \item $N=\binom{n+k}{k}-1$时，$C_{N,k}=\frac{kn}{k+1}(N+1)$；
    \item 区间均摊下界$A_{N,k}\ge\frac{1}{2}(n-1)$。
\end{enumerate}
\end{corollary}

\subsection{8.4 最优策略：二项式计数器}
唯一最优序列对应二项式计数器；维护$b_1\dots b_k$，每次找最小$i$满足$b_i<b_{i+1}$合并进位；全$b_i=n$时策略唯一。

\subsection{8.5 博弈规则拓展}
任意区间合并、无序合并不会降低最小代价，拓展博弈覆盖本文所有块合并/拆分操作。

\section{扩容操作均摊代价下界}
\begin{theorem}
标准实现常驻仅$O(rN^{1/r})$、$r=O(\log N)$时，扩容均摊代价下界$\Omega(r)$。
\end{theorem}
证明：取$k=\Theta(rN^{1/r})$对应博弈最大子数组；二项式放缩证明博弈均摊至少线性$r$；标准实现等价博弈合法操作，故下界$\Omega(r)$。

\section{总结}
本文给出一族可变长数组，完整刻画常驻空间、临时扩缩容空间、增删均摊时间三者最优权衡，同时保证$O(1)$下标访问，几乎完整解决该理论问题。

现有下界仅针对标准实现；引入不可压缩假设可推广至全部数据结构，证明无本质障碍。

扩容博弈具备独立理论价值；未来可探索无需闭式解的归纳下界证明。

% 参考文献 标准bib环境，内容一字不变
\begin{thebibliography}{17}
\bibitem[1]{bille2017fast} Philip Bille, Anders Roy Christiansen, Mikko Berggren Ettienne, and Inge Li Gørtz. Fast dynamic arrays. In Proc. of 25th ESA, volume 87 of LIPIcs, pages 16:1–16:13, 2017.
...
\bibitem[3]{brodnik1997trans} Andrej Brodnik, Peter Bro Miltersen, and J. Ian Munro. Trans-dichotomous algorithms without multiplication - some upper and lower bounds. In Proc. of 4th WADS, volume 1272 of Lecture Notes in Computer Science, pages 426–439. Springer, 1997.
...
\bibitem[6]{fredman1989cell} Michael L. Fredman and Michael E. Saks. The cell probe complexity of dynamic data structures. In David S. Johnson, editor, Proc. of 21st STOC, pages 345–354. ACM, 1989.
...
\bibitem[15]{sitarski1996hat} Edward Sitarski. Algorithm alley: HATs: Hashed array trees. Dr. Dobb’s Journal of Software Tools, 21(9):107–??, September 1996.
\bibitem[16]{wiki-dynarray} Wikipedia contributors. Dynamic array - Wikipedia, the free encyclopedia, 2022.
\bibitem[17]{wiki-hat} Wikipedia contributors. Hashed array tree - Wikipedia, the free encyclopedia, 2022.
\end{thebibliography}
