...
\begin{quote}
    一些补充（后日谈？）:\\
    第二次阅读中疑似写错的——第1部分引言中若扩容系数为$1+\alpha $，内存闲置比例可达$\frac{\alpha}{\alpha+1}$而不是$\frac{1}{\alpha+1}$——在SIAM版本中仍未修改\\
    与同学们讨论后，对第三次阅读问题的解决：\\
    原问题：第1部分中说可以通过拼接两个仅支持单端增删的数组实现Deque，然而这明显是错的，又或者不是显然可构造的\\
    两个仅支持单端增删的数组拼接确实可以很好地支持双端增删，但是如果我像队列那样，一边增，一边删呢？当删的那一端的数组没有元素时就会有问题\\
    解决：当删的那一端的数组没有元素时却仍要进行删除操作时，对整个数组进行重构，使得两个数组都有N/2的元素，复杂度O(N)\\
    易见到下一次重构，中间至少有N/2次删除操作，那么重构均摊复杂度就是O(1)\\
\end{quote}
